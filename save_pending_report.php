...
try {
    ...
    if (!is_array($payload)) {
        throw new RuntimeException('Invalid pending report payload.');
    }
    ...
    if (!is_array($items) || count($items) === 0) {
        throw new RuntimeException('No pending work orders are available to save.');
    }

    $reportId = isset($payload['report_id']) && $payload['report_id'] !== ''
        ? (int) $payload['report_id']
        : null;
    $reportName = 'Pending Work Order List-' . $reportDate;
    ...
    if ($reportId) {
        $statement = $pdo->prepare('
            UPDATE pending_work_order_reports
        ...
        $statement = $pdo->prepare('
            INSERT INTO pending_work_order_reports (report_name, report_date)
    ...
    $pdo->prepare('DELETE FROM pending_work_order_report_items WHERE report_id = ?')->execute([$reportId]);

    $insertItem = $pdo->prepare('
        INSERT INTO pending_work_order_report_items
